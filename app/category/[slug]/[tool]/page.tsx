...
export async function generateMetadata(
  ...
  // Dynamic fallback metaTitle and metaDescription for tools
  const toolName = typeof params?.tool === "string"
    ? params.tool.replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase())
    : "Tool";
  ...
  const fallbackMetaTitle = `${toolName} - Toolinger | Discover and Use Powerful Tools Online`;
  const fallbackMetaDescription = `Discover and use ${toolName} online with Toolinger. Access powerful features, manage your favorites, and enhance your productivity with this tool.`;
...
export default async function ToolDetailsPage({
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Header />
      <div className="py-12">
        <ToolClient tool={params.tool} page={page} />
      </div>
      <Footer />
