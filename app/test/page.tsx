...
const ARTICLES_HTML_DIR = path.join(
  process.cwd(),
  "public",
  "content",
  "tools"
);
const FILENAME = "word_counter_2.html";

export default function TestArticlePage() {
  try {
    const filePath = path.join(ARTICLES_HTML_DIR, FILENAME);
    if (!fs.existsSync(filePath)) {
      return (
        <div style={{ padding: 24 }}>
          Article not found: <b>{FILENAME}</b>
        </div>
      );
    }
    ...
    // extract head (for exported styles) and body
    const headMatch = raw.match(/<head[^>]*>((.|[\r\n])*)<\/head>/im);
    const bodyMatch = raw.match(/<body[^>]*>((.|[\r\n])*)<\/body>/im);

    const headHtml = headMatch ? headMatch[1] : "";
    const bodyHtml = bodyMatch ? bodyMatch[1] : raw; // fallback to raw if no body

    // combine head styles + body content so exported CSS is preserved
    const combined = `${headHtml}\n${bodyHtml}`;

    // sanitize: keep style/class/id/img etc but strip scripts/events
    const clean = sanitizeHtml(combined, {
      ...
      allowedAttributes: {
        ...
        "*": ["class", "style", "id", "title", "role", "aria-hidden"],
      },
    ...
    // THEME OVERRIDES
    // - we DO NOT set a page background (you asked "no background color")
    // - we set color variables so: light => muted gray text, dark => white text
    // - we increase heading sizes and fix list spacing
...
`;
    ...
    return (
      <>
        ...
        <main className="tool-article-wrapper">
          {/* sanitized HTML from export (head styles + body content) */}
    ...
    return (
      <div style={{ padding: 24 }}>
        Internal error reading article. Check server logs.
      </div>
    );
  }
}
