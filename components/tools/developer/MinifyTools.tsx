...
const ToolPage = () => {
  ...
  const currentTool = useMemo(() => {
    ...
    const match = tools.find((t) =>
      pathname?.endsWith(t.route.split("/").pop() || "")
    );
      ...
      <div className="container mx-auto p-4">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
          ...
          <div className="md:col-span-7 col-span-1">
                  ...
                  {showAdvanced && (
                    <div className="mt-4 p-3 bg-gray-200 dark:bg-gray-800 rounded text-sm text-gray-700 dark:text-gray-200">
                      ...
                      <ul className="list-disc ml-5 mt-2">
                        ...
                        <li>
                          Preserve line breaks in &lt;pre&gt; and
                          &lt;textarea&gt;
                        </li>
            ...
            {currentTool == "minify-html" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "beautify-js" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "beautify-php" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "beautify-css" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "beautify-json" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "beautify-xml" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "minify-css" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "minify-js" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
            {currentTool == "minify-json" && (
              <ArticleRenderer
                file={`${currentTool.replace(/-/g, "_")}.html`}
              />
            )}
