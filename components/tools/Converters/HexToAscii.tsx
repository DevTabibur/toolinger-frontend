...
function hexToAscii(hex: string): string {
    // Remove non-hex chars, split every 2 chars
    return hex
        .replace(/[^0-9a-fA-F]/g, "")
        .replace(/(.{2})/g, "$1 ")
        .trim()
...
export default function HexToAscii(props: { article?: any, seo?: any }) {
    const [leftValue, setLeftValue] = useState("");
    const [rightValue, setRightValue] = useState("");
    const [leftType, setLeftType] = useState<"hex" | "ascii">("hex");
    ...
    const handleLoadSample = () => {
        setLeftValue(sampleHex);
        setRightValue(hexToAscii(sampleHex));
        setLeftType("hex");
    };
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-4">
                <nav className="flex items-center space-x-2 text-sm">
                    ...
                    <ChevronRight className="h-4 w-4 text-muted-foreground" />
                    <span className="text-foreground font-medium">Hex to ASCII converter</span>
                </nav>
            </div>
                    ...
                    <div className="md:col-span-7 col-span-1">
                        <div className="border rounded-lg p-4 mb-8 bg-white dark:bg-gray-900 shadow-sm dark:border-gray-700">
                            ...
                            <p className="text-center mb-6 text-gray-600 dark:text-gray-300">
                                To use Toolinger <b>Hex to ASCII</b> Converter, Enter the Hex string below
                            </p>
                            <div className="flex flex-col md:flex-row gap-4 items-start">
                                {/* Left Side: Input */}
                                <div className="flex-1 w-full">
                                    <label className="block mb-2 font-semibold text-gray-700 dark:text-gray-200">
                                        {leftType === "hex" ? "Hex Input" : "ASCII Input"}
                                    </label>
                                    <textarea
                                        ...
                                        placeholder={leftType === "hex" ? "Enter Hex string (e.g. 48656c6c6f)..." : "Enter ASCII text..."}
                                        value={leftValue}
                                ...
                                <div className="flex-1 w-full">
                                    <label className="block mb-2 font-semibold text-gray-700 dark:text-gray-200">
                                        {leftType === "hex" ? "ASCII Output" : "Hex Output"}
                                    </label>
                                    <textarea
                                        className="w-full h-40 p-3 border rounded bg-gray-100 dark:bg-gray-950 dark:text-gray-100 resize-none"
                                        value={rightValue}
                                        readOnly
                                        placeholder={leftType === "hex" ? "ASCII output will appear here..." : "Hex output will appear here..."}
                                        spellCheck={false}
                                    />
                                </div>
                            </div>
