...
// --- ACCURATE BENGALI DATE CONVERSION ---
const BENGALI_MONTHS = [
    { name: "Boishakh", start: { month: 3, day: 14 } },   // April 14
    { name: "Joishtho", start: { month: 4, day: 15 } },   // May 15
    { name: "Ashar", start: { month: 5, day: 15 } },      // June 15
    { name: "Srabon", start: { month: 6, day: 16 } },     // July 16
    { name: "Bhadro", start: { month: 7, day: 17 } },     // August 17
    { name: "Ashwin", start: { month: 8, day: 17 } },     // September 17
    { name: "Kartik", start: { month: 9, day: 18 } },     // October 18
    { name: "Ogrohayon", start: { month: 10, day: 18 } }, // November 18
    { name: "Poush", start: { month: 11, day: 17 } },     // December 17
    { name: "Magh", start: { month: 0, day: 15 } },       // January 15
    { name: "Falgun", start: { month: 1, day: 13 } },     // February 13
    { name: "Chaitra", start: { month: 2, day: 15 } }     // March 15
];
...
function gregorianToBengali(date: Date) {
    ...
    for (let i = 0; i < BENGALI_MONTHS.length; i++) {
        ...
        const nextStartDate = new Date(nextStartYear, BENGALI_MONTHS[nextMonth].start.month, BENGALI_MONTHS[nextMonth].start.day);

        if (date >= startDate && date < nextStartDate) {
            bmonth = i;
            bday = Math.floor((date.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
            break;
        }
    }
    if (bday === 0) {
    ...
    const toBangla = (n: number) => n.toString().split("").map(d => digits[parseInt(d)] ?? d).join("");

    return `${toBangla(bday)} ${BENGALI_MONTHS[bmonth].name}, ${toBangla(byear)}`;
}

// --- ACCURATE HIJRI (ISLAMIC) DATE CONVERSION ---
// Umm al-Qura calendar approximation (tabular, not astronomical, but much more accurate than previous)
function gregorianToHijri(date: Date): string {
    // Use the Kuwaiti algorithm (used in many open-source libraries)
    // Source: https://webspace.science.uu.nl/~gent0113/islam/addfiles/islamiccalendar.htm
    ...
    // The Islamic calendar epoch is July 16, 622 CE (Julian)
    let islamicEpoch = 1948439.5;
    let days = Math.floor(jd) - Math.floor(islamicEpoch);

    let hYear = Math.floor((30 * days + 10646) / 10631);
    let hMonth = Math.min(12, Math.ceil((days - 29 - hijriToJD(hYear, 1, 1) + 1) / 29.5) + 1);
    ...
    // Arabic numerals for day and year
    const digits = "٠١٢٣٤٥٦٧٨٩";
    const toArabic = (n: number) => n.toString().split("").map(d => digits[parseInt(d)] ?? d).join("");
...
// Helper: Convert Gregorian date to Roman numerals
...
function gregorianToRoman(date: Date): string {
    return `${toRoman(date.getDate())} ${toRoman(date.getMonth() + 1)} ${toRoman(date.getFullYear())}`;
}

// Helper: Convert Gregorian date to Hindi (Devanagari numerals)
...
function gregorianToHindi(date: Date): string {
    ...
    return `${toDevanagari(date.getDate())} ${months[date.getMonth()]}, ${toDevanagari(date.getFullYear())}`;
}

// Helper: Convert Gregorian date to English (default)
...
export default function MultiDateConverter() {
    ...
    const handleConvert = () => {
        ...
        // For Arabic, show time in Arabic-Indic numerals
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-4">
                <nav className="flex items-center space-x-2 text-sm">
                    ...
                    <ChevronRight className="h-4 w-4 text-muted-foreground" />
                    <span className="text-foreground font-medium">Multi Date Converter</span>
                </nav>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                    ...
                    <div className="md:col-span-7 col-span-1 bg-white dark:bg-gray-800 rounded shadow p-4">
                        <div>
                            <h1 className="text-2xl font-bold text-center mb-2 text-gray-900 dark:text-gray-100">
                                Multi Date Converter
                            </h1>
                            <p className="text-center mb-6 text-gray-600 dark:text-gray-300">
                                Convert date and time between <b>English (Gregorian)</b>, <b>Bangla (Bengali)</b>, <b>Arabic (Hijri)</b>, <b>Roman Numerals</b>, and <b>Hindi (Devanagari)</b> calendars.
                            </p>
                                ...
                                <div className="flex-1 w-full">
                                    ...
                                    <div className="flex gap-2 mb-2">
                                        <div className="flex-1">
                                            <label className="block text-xs mb-1 text-gray-600 dark:text-gray-300">From</label>
                                            <select
                                            ...
                                            >
                                                {CALENDARS.map(cal => (
                                                    <option key={cal.key} value={cal.key}>{cal.label}</option>
                                                ))}
                                            </select>
                                            <span className="text-xs text-gray-400">(Only Gregorian supported as input for now)</span>
                                        </div>
                                        <div className="flex-1">
                                            <label className="block text-xs mb-1 text-gray-600 dark:text-gray-300">To</label>
                                            <select
                                                className="w-full p-2 border rounded bg-gray-50 dark:bg-gray-900 dark:text-gray-100"
                                                value={toCalendar}
                    ...
                    <div className="md:col-span-5 col-span-1 bg-white dark:bg-gray-800 rounded shadow p-4">
                        ...
                        <div className="text-center text-gray-500 dark:text-gray-300">
                            <h2 className="text-lg font-semibold mb-2">About Multi Date Converter</h2>
                            <p>
                                This tool allows you to convert dates and times between different calendar systems, including Bengali, Arabic (Hijri), Roman numerals, Hindi, and English (Gregorian).
                            </p>
                            <div className="mt-4">
                                <b>Use Cases:</b>
