...
const sampleHex = "48 65 6c 6c 6f";

// Helper: ASCII string to hex string
...
function hexToAscii(hex: string): string {
    // Remove non-hex chars, split by space
    return hex
        .replace(/[^0-9a-fA-F ]/g, "")
        .split(/\s+/)
        .filter(Boolean)
...
export default function AsciiToHEX(props: { article?: any, seo?: any }) {
    const [leftValue, setLeftValue] = useState("");
    const [rightValue, setRightValue] = useState("");
    const [leftType, setLeftType] = useState<"ascii" | "hex">("ascii");
    ...
    const handleToHex = () => {
        setRightValue(asciiToHex(leftValue));
        setLeftType("ascii");
    };
    const handleToAscii = () => {
        setRightValue(hexToAscii(leftValue));
        setLeftType("hex");
    };
    ...
    const handleLoadSample = () => {
        ...
        setRightValue(asciiToHex(sampleAscii));
        setLeftType("ascii");
    };
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-4">
                <nav className="flex items-center space-x-2 text-sm">
                    ...
                    <ChevronRight className="h-4 w-4 text-muted-foreground" />
                    <span className="text-foreground font-medium">ASCII to Hex converter</span>
                </nav>
            </div>
                    ...
                    <div className="md:col-span-7 col-span-1 bg-white dark:bg-gray-800 rounded shadow p-4">
                        <div >
                            ...
                            <p className="text-center mb-6 text-gray-600 dark:text-gray-300">
                                To use prepost <b>ASCII to Hex</b> Converter, Enter the ASCII text below
                            </p>
                            <div className="flex flex-col md:flex-row gap-4 items-start">
                                {/* Left Side: Input */}
                                <div className="flex-1 w-full">
                                    <label className="block mb-2 font-semibold text-gray-700 dark:text-gray-200">
                                        {leftType === "ascii" ? "ASCII Input" : "Hex Input"}
                                    </label>
                                    <textarea
                                        ...
                                        placeholder={leftType === "ascii" ? "Enter ASCII text..." : "Enter Hex (e.g. 48 65 6c 6c 6f)"}
                                        value={leftValue}
                                ...
                                <div className="flex-1 w-full">
                                    <label className="block mb-2 font-semibold text-gray-700 dark:text-gray-200">
                                        {leftType === "ascii" ? "Hex Output" : "ASCII Output"}
                                    </label>
                                    <textarea
                                        className="w-full h-40 p-3 border rounded bg-gray-100 dark:bg-gray-950 dark:text-gray-100 resize-none"
                                        value={rightValue}
                                        readOnly
                                        placeholder={leftType === "ascii" ? "Hex output will appear here..." : "ASCII output will appear here..."}
                                        spellCheck={false}
                                    />
                                </div>
                            </div>
